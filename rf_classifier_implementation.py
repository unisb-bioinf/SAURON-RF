...
# Training data set (for testing purposes only)
# import numpy
# from sklearn.metrics import mean_squared_error
# from sklearn.datasets import make_regression
# from sklearn.ensemble import RandomForestRegressor
# X_train, y_train = make_regression(n_samples=100, n_features=20, n_informative=15, noise=0.1, random_state=2)
# numpy.random.seed(234)
# class_assignment_samples_train = numpy.random.choice(a=[0, 1], size=100, p=[0.6, 0.4])
# column_names_train = ["feature_" + str(i) for i in range(0, 20)]
# sample_names_train = ["sample_" + str(i) for i in range(0, 100)]
# number_of_trees_in_forest = 14
# min_number_of_samples_per_leaf = 5
# number_of_features_per_split = math.floor(float(X_train.shape[1]) / 3.0)

# X_test, y_test = make_regression(n_samples=20, n_features=20, n_informative=15, noise=0.1, random_state=2)
# class_assignment_samples_test = numpy.random.choice(a=[0, 1], size=20, p=[0.6, 0.4])
# sample_names_test = ["te_sample_" + str(i) for i in range(0, 20)]

# model = RandomForestRegressor(n_estimators=14, max_depth=2, random_state=2)
# model.fit(X_train, y_train)

# tree0 = model.estimators_[0]

# applied_tree0 = tree0.apply(X_train)
# fit model

# Make a standard prediction with the whole forest
# pred_whole_forest = model.predict(X_test)

# all_feature_imps = model.feature_importances_
# for idx in range(0,20):

#    if all_feature_imps[idx] != 0.0:

#        print(idx)

# mse = mean_squared_error(y_pred=pred_whole_forest, y_true=y_test)

# Make predictions with specific trees (using all trees now just to check)
# Gave the same result
# single_tree_predictions = []
# for tree_number in tree_to_leaf_to_percentage_class_dict.keys():
#   tree_x = model.estimators_[tree_number]

#  pred_single_tree = tree_x.predict(X_test)
# single_tree_predictions.append(pred_single_tree)

# final_prediction_manuallly_calculated = []

# for sample in range(0, X_test.shape[0]):

#   total_pred = 0.0

#  for tree in tree_to_leaf_to_percentage_class_dict.keys():

#     total_pred = total_pred + single_tree_predictions[tree][sample]

# total_pred = total_pred / number_of_trees

# final_prediction_manuallly_calculated.append(total_pred)

# Making predictions only with trees from majority class for each sample
# 'binary' option
