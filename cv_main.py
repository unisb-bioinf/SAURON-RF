import harf_implementation
import rf_classifier_implementation
import sys
import json
import numpy
import math
...
def start_harf_method(json_dict):
	...
	output_sample_prediction_file_train = output_directory + name_of_analysis + "_Training_Set_Predictions.txt"
	output_sample_prediction_file_test = output_directory + name_of_analysis + "_Test_Set_Predictions.txt"
	train_error_file = output_directory + name_of_analysis + "_" + "Train_Error.txt"
	test_error_file = output_directory + name_of_analysis + "_" + "Test_Error.txt"
	...
	output_leaf_purity_file_train = output_directory + name_of_analysis + "_Training_Set_LeafPurity.txt"
	output_leaf_purity_file_test = output_directory + name_of_analysis + "_Test_Set_LeafPurity.txt"
	output_variance_file_train = output_directory + name_of_analysis + "_Training_Set_Variance.txt"
	output_variance_file_test = output_directory + name_of_analysis + "_Test_Set_Variance.txt"
	leaf_assignment_file_train = output_directory + name_of_analysis + "_Training_Set_LeafAssignment.txt"
	leaf_assignment_file_test = output_directory + name_of_analysis + "_Test_Set_LeafAssignment.txt"
	sample_info_file = output_directory + name_of_analysis + "_Additional_Sample_Information.txt"
	debug_file = output_directory + name_of_analysis + "_Debug.txt"
	classification_single_tree_output_file_train = output_directory + name_of_analysis + "_Train_Class_Predictions_Per_Tree.txt"
	classification_single_tree_output_file_test = output_directory + name_of_analysis + "_Test_Class_Predictions_Per_Tree.txt"

	threshold_g = json_dict["threshold"]
	threshold = 0.0
	if threshold_g !="":

		threshold = float(threshold_g)
	else:
		threshold = float("NaN")
	...
	if not "regression_classification" in json_dict:
		...
		harf_implementation.train_and_test_rf_model(json_dict["analysis_mode"], X_train, X_test, y_train, y_test, samples_names_train, samples_names_test,
												...
												number_of_features_per_split, class_assignment_samples_train, class_assignment_samples_test, name_of_analysis, mse_included, classification_included, feature_imp_output_file,
												feature_names, threshold, upsampling, time_file, sample_weights_included, percentage_top_candidates, output_leaf_purity_file_train, output_leaf_purity_file_test, output_variance_file_train, output_variance_file_test, leaf_assignment_file_train, leaf_assignment_file_test, sample_info_file, debug_file)
	elif json_dict["regression_classification"] == "regression":
		harf_implementation.train_and_test_rf_model(json_dict["analysis_mode"], X_train, X_test, y_train, y_test,
...
if __name__ == "__main__":
	...
	if len(sys.argv) < 2:
			sys.exit("This program needs the following arguments:\
					...
					\n d) analysis mode for HARF (analysis_mode, accepted: binary_no_weights, binary_weights, majority_weights, vanilla, binary_top_percentage)\
					...
					\n o) Threshold for calculating weights (threshold). Can be used with sample weights or upsampling (proportional upsampling).\
					\n p) Should training data be upsampled for minority class (upsample, accepted: simple, proportional). Cannot be combined with sample_weights = linear, quadratic, simple\
					\n q) If binary_top_percentage is used as analysis mode, the percentage of trees you would like to consider, should be given (percentage_top_candidates)\
					\n r) Should sample weights be used for fitting trees? (sample_weights, accepted: simple, linear, quadratic, no)\
					\n s) Information whether classification or regression trees should be fit (regression_classification, accepted: regression, classification)")
